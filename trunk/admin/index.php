...
function edit_file_content($content='', $filename='')
{
	...
	$sform = new XoopsThemeForm(_AM_BOOX_FILE, 'ffile', XOOPS_URL.'/modules/boox/admin/index.php', 'post');
...
switch ($op) {
	...
	case 'save':		// Save a file after it was edited (or created)
		...
		if(!isset($_POST['filename'])) {
			redirect_header( 'index.php', 3, _AM_BOOX_ERROR1);
		}
		...
		redirect_header('index.php',2,_AM_BOOX_DBUPDATED);
		break;
	...
	default:		// Default action, list existing files
		...
		foreach($files as $file) {
			...
			$action_delete = '<a '.boox_JavascriptLinkConfirm(_AM_BOOX_CONFIRM).' title="'._DELETE.'" href="index.php?op=delete&filename='.urlencode($file).'"><img border="0" src="../images/delete.gif" /></a>';
			$action_edit = '<a title="'._EDIT.'" href="index.php?op=edit&filename='.urlencode($file).'"><img border="0" src="../images/edit.gif" /></a>';
			$command = "<{php}>include_once '".$folder_storage.$file."';<{/php}>";
			echo "<tr class='".$class."'>";
			echo '<td>'.$file.'</td>';
			echo '<td>'.$command.'</td>';
			echo "<td align='center'>".$action_delete.' '.$action_edit.'</td>';
			echo "</tr>";
		}
		$class = ($class == 'even') ? 'odd' : 'even';
		echo "<tr class='".$class."'><td colspan='3' align='center'><br /><form method='post' name='fadd' id='fadd' action='index.php'><input type='hidden' name='op' value='addfile' /><input type='submit' name='btngo' value='"._AM_BOOX_ADD."' /></form><br /></td></tr>";
		echo '</table>';
